...
def load_pickle(f) -> Any:
    return pickle.load(f, encoding="latin1")


def load_CIFAR_batch(filename) -> Tuple[np.ndarray, np.ndarray]:
    """each batch contains the following elements:
    ...
    the next 1024 the green, and the final 1024 the blue. The image is stored in row-major order, so that the first 32 entries of the array are the red
